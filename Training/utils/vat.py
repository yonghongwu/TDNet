...
class VAT(nn.Module):
    ...
    def forward(self, X, logit, eval=False):
        if eval is True:
            vat_loss = virtual_adversarial_loss(X, logit, self.model, self.n_power,
                                                self.XI, self.epsilon)
        else:
            assert eval == False, 'check'
            vat_loss = virtual_adversarial_loss(X, logit, self.model.eval(), self.n_power,
                                                self.XI, self.epsilon)
...
def generate_virtual_adversarial_perturbation(x, logit, model, n_power, XI,
    ...
    with _disable_tracking_bn_stats(model): # by wyh copy from VAT-pytorch
